 ...
 * @author Nico_ND1
 */
public class ChunkUnclaimCommand implements ICommand<Player> {
    @Override
    ...
    public Result onExecute(Player player, String label, String[] args) {
        Factions.getInstance().getDatabaseSaver().findFaction(new FactionsPlayer(player), optionalFaction -> {
            ...
            chunkManager.getFaction(claimableChunk, optionalFaction1 -> {
                ...
                if(!faction1.getName().equalsIgnoreCase(faction.getName())) {
                    player.sendMessage(Messages.FACTION_CHUNK_NOT_OWN.getMessage());
                    return;
                }

                chunkManager.unclaimChunk(faction, claimableChunk, () -> player.sendMessage(Messages.FACTION_CHUNK_CLAIMED.getMessage()));
            });
        });
